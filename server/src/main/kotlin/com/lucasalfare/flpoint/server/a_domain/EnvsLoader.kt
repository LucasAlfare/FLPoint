...
import com.lucasalfare.flpoint.server.a_domain.model.EmptyEnvironmentVariableError
import com.lucasalfare.flpoint.server.a_domain.model.NullEnvironmentVariableError
...
data class EnvValue(
  ...
  private val value = System.getenv(name).let {
    when {
      it == null -> {
        if (throwWhenNull)
          throw NullEnvironmentVariableError("Server environment variable [$name] is missing/null")
        defaultWhenNull
      }

      it.isEmpty() -> {
        if (throwWhenEmpty)
          throw EmptyEnvironmentVariableError("Server environment variable [$name] is missing/empty")
        defaultWhenEmpty
      }
